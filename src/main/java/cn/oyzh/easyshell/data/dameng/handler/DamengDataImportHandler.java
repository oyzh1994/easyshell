...
public class DamengDataImportHandler extends DBDataImportHandler<String> {
     ...
     * 导入文件
     */
    private List<ShellDamengDataImportFile> files;
    ...
    public void doImport() throws Exception {
        ...
        if (CollectionUtil.isNotEmpty(this.files)) {
            for (ShellDamengDataImportFile file : files) {
                this.checkInterrupt();
        ...
        try (DamengTypeFileReader reader = this.initReader(file.getFile())) {
            ...
            if (!dbColumns.isEmpty()) {
                while (true) {
                    ...
                    List<DamengRecord> records = this.readRecords(reader, this.readLimit);
    ...
    private List<DamengRecord> readRecords(DamengTypeFileReader reader, int count) throws Exception {
        ...
        for (Map<String, Object> objectMap : list) {
            DamengRecord record = new DamengRecord(null);
