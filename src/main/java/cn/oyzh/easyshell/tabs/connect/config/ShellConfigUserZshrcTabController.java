 ...
 * @since 2025/03/18
 */
public class ShellConfigUserZshrcTabController extends SubTabController {
    ...
    private void save() {
        ...
        StageManager.showMask(() -> {
            ...
            try (ShellSftp sftp = this.client().newSftp()) {
                // 创建临时文件
                String tempFile = this.client().getUserHome() + ".zshrc.temp";
    ...
    private void apply() {
        ShellExec exec = this.client().shellExec();
        StageManager.showMask(() -> {
            String output = exec.source("~/.zshrc");
            if (!StringUtil.isBlank(output)) {
                MessageBox.warn(output);
            }
        });
    ...
    public void onTabInit(RichTab tab) {
        super.onTabInit(tab);
        this.userZshrc.selectedProperty().addListener((observableValue, aBoolean, t1) -> {
            if (t1) {
                this.refresh();
            }
        });
