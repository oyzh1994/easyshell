 ...
 * @since 2024/07/08
 */
public class DamengProcedureDesignTabController extends RichTabController {
     ...
     * @param dbItem    db库树节点
     */
    public void init(DamengProcedure procedure, DamengSchemaTreeItem dbItem) {
        this.dbItem = dbItem;
    ...
    private void doSave() {
        try {
            ...
            if (this.newData) {
                ...
                DamengEventUtil.procedureAdded(this.dbItem);
                this.initDBListener();
            } else {// 修改过程
                this.dbItem.alertProcedure(tempProcedure);
                DamengEventUtil.procedureAlerted(this.procedureName, this.dbItem);
            }
            // 更新保存标志位
            this.unsaved = false;
            // 更新新数据标志位
