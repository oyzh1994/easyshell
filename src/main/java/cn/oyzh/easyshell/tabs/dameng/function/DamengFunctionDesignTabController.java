 ...
 * @since 2024/07/08
 */
public class DamengFunctionDesignTabController extends RichTabController {
     ...
     * @param dbItem   db库树节点
     */
    public void init(DamengFunction function, DamengSchemaTreeItem dbItem) {
        this.dbItem = dbItem;
    ...
    private void doSave() {
        try {
            ...
            if (this.newData) {
                ...
                DamengEventUtil.functionAdded(this.dbItem);
                this.initDBListener();
            } else {// 修改过程
                this.dbItem.alertFunction(tempFunction);
                DamengEventUtil.functionAlerted(this.functionName, this.dbItem);
            }
            // 更新保存标志位
            this.unsaved = false;
            // 更新新数据标志位
