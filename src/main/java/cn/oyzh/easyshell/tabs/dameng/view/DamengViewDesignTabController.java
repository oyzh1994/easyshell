 ...
 * @since 2024/06/28
 */
public class DamengViewDesignTabController extends RichTabController {
     ...
     * @param dbItem db库树节点
     */
    public void init(DamengView view, DamengSchemaTreeItem dbItem) {
        this.dbItem = dbItem;
        this.view = view;
        // 更新新数据标志位
    ...
    private void doSave() {
        try {
            ...
            if (this.newData) {
                ...
                DamengEventUtil.viewAdded(this.dbItem);
                // 初始化监听器
                this.initDBListener();
            } else {// 修改视图
                this.dbItem.alertView(tempView);
                DamengEventUtil.viewAlerted(viewName, this.dbItem);
            }
            // 重置保存标志位
            this.unsaved = false;
            // 更新新数据标志位
