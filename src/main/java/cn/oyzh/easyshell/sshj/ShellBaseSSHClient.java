...
import cn.oyzh.easyshell.ssh.ShellSSHUtil;
 ...
 * @since 2023/08/16
 */
public class ShellBaseSSHClient implements BaseClient {
    ...
    public ShellBaseSSHClient(ShellConnect shellConnect) {
     ...
     * 初始化客户端
     */
    private void initClient(int timeout) throws IOException {
    ...
    public Session.Shell openShell() {
        if (this.shell == null || !this.shell.isOpen()) {
            try {
                ...
                this.shell = this.getSession().startShell();
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        }
