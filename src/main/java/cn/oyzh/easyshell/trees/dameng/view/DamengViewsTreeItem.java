 ...
 * @since 2023/12/08
 */
public class DamengViewsTreeItem extends DBTreeItem<DamengViewsTreeItemValue> {

    public DamengViewsTreeItem(RichTreeView treeView) {
        super(treeView);
        super.setFilterable(true);
        this.setValue(new DamengViewsTreeItemValue(this));
    }
    ...
    private void add() {
        ...
        DamengEventUtil.designView(dbView, this.parent());
    }
    ...
    public void loadChild() {
        if (!this.isWaiting() && !this.isLoaded() && !this.isLoading()) {
            ...
            Task task = TaskBuilder.newBuilder()
                    .onStart(() -> {
                        List<DamengView> views = this.client().selectViewsSimple(this.schema());
                        // 无数据直接更新列表
                        if (this.isChildEmpty()) {
                            List<TreeItem<?>> list = new ArrayList<>();
                            for (DamengView view : views) {
                                list.add(new DamengViewTreeItem(view, this.getTreeView()));
                            }
                            this.setChild(list);
                        } else {// 有数据则执行删除、新增、更新操作
                            ObservableList children = this.richChildren();
                            ObservableList<DamengViewTreeItem> list = children;
                            List<DamengViewTreeItem> delList = new ArrayList<>();
                            List<DamengViewTreeItem> addList = new ArrayList<>();
                            // 删除
                            for (DamengViewTreeItem item : list) {
                                if (views.parallelStream().noneMatch(f -> f.compare(item.value()))) {
                                    delList.add(item);
                                }
                            }
                            // 新增
                            for (DamengView f : views) {
                                if (list.parallelStream().noneMatch(item -> f.compare(item.value()))) {
                                    addList.add(new DamengViewTreeItem(f, this.getTreeView()));
                                }
                            }
                            // 更新
                            for (DamengViewTreeItem item : list) {
    ...
    public void addView(DamengView view) {
        this.addChild(new DamengViewTreeItem(view, this.getTreeView()));
