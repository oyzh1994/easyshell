...
public class ShellDamengTableTreeItem extends DBTreeItem<ShellDamengTableTreeItemValue> {
    ...
    public ShellDamengTableTreeItem(DamengTable table, RichTreeView treeView) {
        super(treeView);
        this.value = table;
        this.setValue(new ShellDamengTableTreeItemValue(this));
    }

    @Override
    public ShellDamengTablesTreeItem parent() {
    ...
    private void truncateTable() {
        if (MessageBox.confirm(I18nHelper.truncateTable() + "[" + this.tableName() + "]")) {
            try {
                ...
                ShellDamengEventUtil.tableTruncated(this, this.dbItem());
            } catch (Exception ex) {
                ex.printStackTrace();
                MessageBox.exception(ex);
            }
    ...
    private void clearTableData() {
        try {
            if (MessageBox.confirm(I18nHelper.clearTableData() + "[" + this.tableName() + "]")) {
                ...
                ShellDamengEventUtil.tableCleared(this, this.dbItem());
            }
        } catch (Exception ex) {
            MessageBox.exception(ex);
        }
    ...
    public void rename() {
        try {
            ...
            this.refresh();
            ShellDamengEventUtil.tableRenamed(oldName, tableName, this.dbItem());
        } catch (Exception ex) {
            ex.printStackTrace();
            MessageBox.exception(ex);
        }
