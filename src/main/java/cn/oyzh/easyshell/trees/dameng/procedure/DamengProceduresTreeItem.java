 ...
 * @since 2024/06/29
 */
public class DamengProceduresTreeItem extends DBTreeItem<DamengProceduresTreeItemValue> {

    public DamengProceduresTreeItem(RichTreeView treeView) {
        super(treeView);
        super.setFilterable(true);
        this.setValue(new DamengProceduresTreeItemValue(this));
    }
    ...
    private void add() {
        ...
        DamengEventUtil.designProcedure(procedure, this.parent());
    }
                    ...
                    .onStart(() -> {
                        ...
                        if (this.isChildEmpty()) {
                            ...
                            for (DamengProcedure procedure : procedures) {
                                list.add(new DamengProcedureTreeItem(procedure, this.getTreeView()));
                            }
                            this.setChild(list);
                        } else {// 有数据则执行删除、新增、更新操作
                            ObservableList children = this.richChildren();
                            ObservableList<DamengProcedureTreeItem> list = children;
                            List<DamengProcedureTreeItem> delList = new ArrayList<>();
                            List<DamengProcedureTreeItem> addList = new ArrayList<>();
                            // 删除
                            for (DamengProcedureTreeItem item : list) {
                            ...
                            for (DamengProcedure f : procedures) {
                                if (list.parallelStream().noneMatch(item -> f.compare(item.value()))) {
                                    addList.add(new DamengProcedureTreeItem(f, this.getTreeView()));
                                }
                            }
                            // 更新
                            for (DamengProcedureTreeItem item : list) {
    ...
    public void addProcedure(DamengProcedure procedure) {
        this.addChild(new DamengProcedureTreeItem(procedure, this.getTreeView()));
