...
public class ZooKeeper {
    ...
    static class ZKWatchManager implements ClientWatchManager {
        ...
        public Map<EventType, Set<Watcher>> removeWatcher(String clientPath,
            ...
            if (!removedWatcher) {
                throw new NoWatcherException(clientPath);
            }
        ...
        */
        void containsWatcher(String path, Watcher watcher,
            ...
            // Watcher function doesn't exists for the specified params
            if (!containsWatcher) {
                throw new NoWatcherException(path);
            }
        }
    ...
    public void multi(Iterable<Op> ops, MultiCallback cb, Object ctx) {
        ...
        if (results.size() > 0) {
            cb.processResult(Code.BADARGUMENTS.intValue(),
                    null, ctx, results);
            return;
        }
    ...
    private List<OpResult> validatePath(Iterable<Op> ops) {
        ...
        for (Op op : ops) {
            ...
            ErrorResult err = new ErrorResult(
                    Code.RUNTIMEINCONSISTENCY.intValue());
            results.add(err);
        }
        if (false == error) {
            results.clear();
    ...
    public int getAllChildrenNumber(final String path) throws KeeperException, InterruptedException {
        ...
        ReplyHeader r = cnxn.submitRequest(h, request, response, null);
        if (r.getErr() != 0) {
            throw KeeperException.create(Code.get(r.getErr()), clientPath);
        }
