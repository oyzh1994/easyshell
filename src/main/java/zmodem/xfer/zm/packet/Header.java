...
public class Header extends ZMPacket {
    ...
    public static Header unmarshall(Buffer buff) {
        ...
        zmodem.xfer.zm.packet.Format fmt = null;

        while (fmt == null)
            fmt = zmodem.xfer.zm.packet.Format.fromByte(buff.get());
    ...
    private zmodem.xfer.zm.packet.Format format;
    private ZModemCharacter type;
    private byte[] data = {0, 0, 0, 0};

    private Header(zmodem.xfer.zm.packet.Format fFmt) {
        format = fFmt;
    }

    public Header(zmodem.xfer.zm.packet.Format fFmt, ZModemCharacter fType) {
        this(fFmt);
        type = fType;
    }

    public Header(zmodem.xfer.zm.packet.Format fFmt, ZModemCharacter fType, byte[] flags) {
        this(fFmt, fType);
        setFlags(flags);
    }

    public Header(zmodem.xfer.zm.packet.Format fFmt, ZModemCharacter fType, int pos) {
        this(fFmt, fType);
        setPos(pos);
    }
